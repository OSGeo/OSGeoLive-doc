...
Now select ``File --> Download GRIB`` and select the features you want to
download. If in doubt the defaults are ok. When ready click the ``Download
GRIB file`` button at the bottom, and save the file to disk.
...
Select ``Data --> Cloud Cover`` and the view should change.
...
The `OpenCPN software <../overview/opencpn_overview.rst>`_ (also on this
disc) has a GRIB overlay plugin which you may try with your newly
downloaded GRIB file. See
the `OpenCPN quickstart <../quickstart/opencpn_overview.rst>`_ for details.
