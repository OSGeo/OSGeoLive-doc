...
# script assumes it is being run in the main directory
...
if [ -d "$1" ] ; then
  ...
  if [ "$2" ] ; then
    ...
    echo "You have to pass to variables, a source and a target directories"
    exit
  fi
...
cp -p images/banner.png "$target/../../_images/"
